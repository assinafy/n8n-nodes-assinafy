...
The node enforces PDF-only, non-empty, and the 25MB upload limit before sending. Sent as `multipart/form-data` with `file` (the PDF blob) and `name`. Note: `metadata` is appended to the form but is not persisted/returned by the API (silent no-op).
...
[
  {
    ...
    "assignment": null,
    "pages": [
      { "id": "...", "number": 1, "height": 417, "width": 417, "download_url": "https://.../pages/<pid>/download" }
    ],
...
- Signers — fixedCollection (multiple) — required — one entry per template role, each with:
  ...
  - Signer ID — string — optional — existing signer ID (required by the API for create; can be omitted for Estimate Cost).
  - Verification Method — options — optional — `Email` or `Whatsapp` (default `Email`).
  - Notification Methods — multiOptions — optional — `Email` and/or `Whatsapp` (default `Email`).
...
```json
{
  ...
  "assignment": {
    ...
    "expires_at": "2026-07-30T23:59:00Z",
    "signers": [
      { "id": "fa8c140cb49b79f940aab95fddd", "full_name": "Suzana Cordeiro", "email": "suzana@example.com", "has_accepted_terms": false }
    ],
    "copy_receivers": [],
    "summary": { "signer_count": 1, "completed_count": 0, "signers": [{ "id": "fa8c140cb49b79f940aab95fddd", "completed": false }] },
    "signing_urls": [{ "signer_id": "fa8c140cb49b79f940aab95fddd", "url": "https://.../sign/..." }]
  },
  "tags": [{ "id": "ab12cd34...", "name": "Onboarding" }],
  "pages": [{ "id": "fa8c140c9617a07be842995d4a1", "number": 1, "height": 2100, "width": 1275, "download_url": "https://.../download" }],
  "is_closed": false,
  "decline_reason": null,
  "declined_by": null,
...
**Node parameters:**
- Template ID — string — required — the template to use.
- Signers — fixedCollection (multiple) — required — same row shape as Create From Template (only `role_id`, `verification_method`, and `notification_methods` matter for cost; `id` and `step` are accepted but ignored by the API).

The node sends only `signers` (built the same way as create); it validates steps client-side but does not send the additional fields.
...
```json
{
  ...
  "total_credits": 0.45,
  "breakdown": [
    { "code": "NotificationWhatsapp", "name": "Whatsapp Notification", "cost": 0.45, "quantity": 1, "unit_cost": 0.45 }
  ],
...
**Example response:**
```json
{ "detached": true, "documentId": "60f720572d7fecf7c16c8463", "tagId": "fa8c09f3e709a8a1c82d69b1454" }
```
...
Signer-side flow: marks the terms of use as accepted for the signer identified by the access code. Authenticated only by the access code.
...
Signer-side flow: verifies the OTP delivered to the signer (via email or WhatsApp) for the document access code. Authenticated only by the access code.
...
#### Signer confirms email or whatsapp before signing

Signer-side flow: confirms (and, if not yet set, saves) the signer's email and/or WhatsApp number for a specific document before they may sign. Can optionally accept terms in the same call. The access code is sent as a query param; provided fields go in the body. Authenticated only by the access code.
...
- Confirm Fields — collection — optional:
  - Email — string — required when the signer uses email verification/notification; must match an existing email if already set.
  - WhatsApp Phone Number — string (`whatsapp_phone_number`) — required when the signer uses WhatsApp verification/notification; E.164.
  - Accept Terms — boolean (`has_accepted_terms`) — also accept terms of use in this request.

**Example request:**
```json
{
  "qs": { "signer-access-code": "9uAWyOXx9hgzCKdCuahkinwvg8tWJ2RC-n6hhxyLS1QfMhqWSw-PnwQlSs2oPNea" },
  "body": {
    "email": "signer@example.com",
    "whatsapp_phone_number": "+5548999990000",
    "has_accepted_terms": true
...
Signer-side flow: uploads the signer's signature or initials image. The image binary is sent as the raw request body with a `Content-Type` of `image/png` or `image/jpeg` (uploads of other mime types are rejected by the node). The access code and `type` are query params. Authenticated only by the access code.
...
```json
{
  "qs": { "signer-access-code": "9uAWyOXx9hgzCKdCuea", "type": "signature" },
...
```json
{
  "signer-access-code": "9uAWyOXx9hgzCKdCuea"
}
```
  ...
  "items": [
    {
      ...
      "signer": { "id": "61521202f665dffcef5f6b24", "full_name": "Kennith Kuphal", "email": "signer@example.com" },
      "field": { "id": "6152120297080d55bdd13197", "name": "Signature", "type": "signature" },
      "display_settings": { "top": 282, "left": 69, "fontSize": 18, "fontFamily": "Arial", "backgroundColor": "rgb(185, 218, 255)" },
      "value": null,
      "completed": false
    }
  ],
  "summary": { "signer_count": 1, "completed_count": 0, "signers": [{ "id": "61521202f665dffcef5f6b24", "completed": false }] },
  "signing_urls": [
...
```json
{
    ...
    "items": [
      {
        ...
        "page": { "id": "615213ed81b071f4293b2fc2", "number": 1, "height": 2100, "width": 1275, "download_url": "https://api.assinafy.com.br/v1/documents/615213edf8a58f132e1b2384/pages/615213ed81b071f4293b2fc2/download" },
        "signer": { "id": "615212039529a822e24b6913", "full_name": "Sonny Bayer", "email": "sonny@example.com", "has_accepted_terms": true },
        "field": { "id": "6152120297080d55bdd13197", "name": "Signature", "type": "signature" },
        "display_settings": { "top": 285, "left": 639, "width": 501, "height": 27.34, "fontSize": 18, "fontFamily": "Arial", "backgroundColor": "rgb(195, 230, 203)" },
        "value": null,
        "completed": false
      }
    ],
    "summary": { "signer_count": 2, "completed_count": 1, "signers": [{ "id": "customid1", "completed": true }] },
  ...
  "pages": [{ "id": "615213ed81b071f4293b2fc2", "number": 1, "height": 2100, "width": 1275, "download_url": "https://api.assinafy.com.br/v1/documents/615213edf8a58f132e1b2384/pages/615213ed81b071f4293b2fc2/download" }],
  "created_at": 1632769005,
  "updated_at": 1632769005,
  "current_signer": { "id": "615212039529a822e24b6913", "full_name": "Till Man", "email": "tillman@example.com", "has_accepted_terms": true, "verification_method": "Email", "notification_methods": ["Email"] }
}
```
...
```json
{
  ...
  "method": "virtual",
  "signers": [
    { "id": "customid1", "full_name": "Signer 1", "email": "signer1@example.com", "has_accepted_terms": false }
  ],
  "items": [
    {
      "id": "1",
      "page": { "id": "1", "number": 1, "height": 1, "width": 1, "download_url": "https://api.assinafy.com.br/v1/documents/1/pages/1/download" },
      "signer": { "id": "customid1", "full_name": "Signer 1", "email": "signer1@example.com", "has_accepted_terms": false },
      "field": null,
      "display_settings": "",
      "value": "",
      "completed": true
  ...
  "summary": {
    ...
    "completed_count": 1,
    "signers": [
      { "id": "customid1", "full_name": "Signer 1", "email": "signer1@example.com", "has_accepted_terms": false, "completed": true },
      { "id": "customid2", "full_name": "Signer 2", "email": "signer2@example.com", "has_accepted_terms": false, "completed": false }
    ]
  }
}
```
...
```json
{
  "query": { "signer-access-code": "9uAWyOXx9hgzCKdCuahkinwvg8tWJ2RC-n6hhxyLS1QfMhqWSw-PnwQlSs2oPNea" },
  "body": [
    {
...
```json
{
  "query": { "signer-access-code": "1e7d51e68a7" },
...
Validates a single value against a field definition. Can run as an authenticated user (Bearer token) or as a signer using a signer access code. When a Signer Access Code is provided, the SDK skips bearer auth and sends the code as the `signer-access-code` query param.
...
- Signer Access Code — string (password) — optional — Required when validating as a signer instead of an authenticated user.
...
```
Signer-side, the access code is appended as a query parameter: `?signer-access-code=<code>` (Bearer auth is omitted).
...
Validates multiple `{field_id, value}` pairs in a single request. Supports authenticated-user or signer-side access (same `signer-access-code` behavior as Validate). The SDK returns the results wrapped under a `results` key.
...
```
Signer-side, the access code is appended as `?signer-access-code=<code>` (Bearer auth is omitted). The request body is the bare JSON array (not wrapped in an object).
...
[
  {
    ...
    "payload": {
      ...
      "subject": { "id": "def456", "name": "John Doe", "type": "user" },
...
```json
{
  "signer-access-code": "1ca4006b9e111ca4006b9e11"
}
```
...
```json
{
  ...
  "assignment": {
    ...
    "items": [
      {
        "id": "dbd199996981d",
        "signer": { "id": "62d6ee35c7741ca4006b9e11", "full_name": "Signer Name", "email": "[email protected]", "has_accepted_terms": false },
...
[
  {
    ...
    "assignment": {
      ...
      "message": null,
      "signers": [
        { "id": "customid1", "full_name": "Signer 1", "email": "[email protected]", "has_accepted_terms": false },
        { "id": "customid2", "full_name": "Signer 2", "email": "[email protected]", "has_accepted_terms": false }
      ],
      "summary": {
        "signer_count": 2,
        "completed_count": 1,
        "signers": [
          { "id": "dbd199996981d", "full_name": "Signer 1", "email": "[email protected]", "has_accepted_terms": false, "completed": true },
          { "id": "dbd199996981d", "full_name": "Signer 2", "email": "[email protected]", "has_accepted_terms": false, "completed": false }
        ]
      }
    },
    "artifacts": {
...
```
Query: `?signer-access-code=9uAWyOXx9hgzCKhkinwvg8tWJ2RC`
...
Generates a new API key for the user. Requires a Bearer access token (from Login or Social Login). Note: generating a new key deletes the previous one. The returned key is shown in full only here — use it via the `X-Api-Key` header. Never use your API key from a frontend application.

**Endpoint:** `POST /users/api-keys`

**Node parameters:**
- Access Token (`accessToken`) — string (password) — required — JWT access token from Login or Social Login, sent as `Authorization: Bearer <token>`.
- Password (`authPassword`) — string (password) — required — user's password.
...
Revokes the current API key. Requires a Bearer access token. The SDK returns a synthesized confirmation object regardless of the API's empty response.
...
Changes the authenticated user's password. Requires a Bearer access token plus the current and new passwords.
