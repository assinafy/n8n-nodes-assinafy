...
GET requests and bodyless DELETE operations send no request body unless their section explicitly specifies one. Authenticated requests use `Accept: application/json` and either `X-Api-Key: <api-key>` or `Authorization: Bearer <oauth-access-token>`; JSON bodies add `Content-Type: application/json`. Multipart content types and boundaries are generated by the HTTP client. The endpoint paths below are relative to `/v1` unless an absolute URL is shown. With Return All enabled, requests add `page` starting at 1 and `per-page=100`; without it, Limit defaults to 50 and is clamped to 1–100. Other list endpoints describe their own output wrapper. OAuth flat responses are documented separately.
...
An n8n workflow therefore drives the signer-side operations only when a code arrives through a step you control — an inbox-reading node, a WhatsApp integration, or a human pasting it in. Use the API-key credential for owner administration, and the OAuth credential for operations permitted by its scopes. See [OAuth](#oauth) for token-specific requirements.
...
If the document omits its assignment or the counts needed to compute progress, `available` is `false`; `signed`, `total`, `pending`, and `percentage` are `null`. `isFullySigned` remains `true` when the document status is `certificated`.
...
Convenience wrapper: searches all result pages for an exact (case-insensitive) email match and returns the matching signer. If none is found, the SDK returns a synthesized `{ "found": false, "email": ... }` object instead of erroring.
...
Signer-side flow: marks the terms of use as accepted for the signer identified by the access code. The request puts the access code in the query string and has no body.
...
The API exposes signer rejection and document deletion in permitted statuses. It does not define a separate assignment cancellation endpoint.
...
For normal workflows, select **Authentication → OAuth2** and use the **Assinafy OAuth2 API** credential as described in the [connection guide](../README.md#oauth-for-document-workflows). n8n handles consent, PKCE, encrypted token storage, and renewal. These five operations expose the protocol endpoints for workflows that explicitly manage their own OAuth connection. Raw tokens returned by these operations are sensitive execution data: restrict execution retention and access, and never pin or export token-bearing output.

Production authorization server: `https://auth.assinafy.com.br`. Its discovery document is `https://auth.assinafy.com.br/.well-known/oauth-authorization-server`. The API origin is `https://api.assinafy.com.br`; token, revocation, and UserInfo endpoints are below `/v1`. The protected-resource metadata endpoint is at the origin, outside `/v1`.
...
| Client Secret | `client_secret` | Required for confidential applications; omit for public applications |
| Authorization Code | `code` | Required, unused code from the callback |
| Redirect URI | `redirect_uri` | Required, exact registered HTTPS callback; no user info or fragment |
| Code Verifier | `code_verifier` | Required, original 43–128 characters from `A–Z a–z 0–9 - . _ ~` |
| Resource Indicator | `resource` | Optional, must match authorization |
...
`refresh_token` is present only when `offline_access` was requested and granted. `id_token` is present only with `openid`; it is a signed RS256 OIDC token and requires signature, issuer, audience, expiry, and applicable nonce validation before trusting its claims. Optional token fields may be absent or null. `scope` describes the access-token permissions and excludes the request-only `offline_access` signal. The node preserves the response without extracting or logging tokens.
...
**Node parameters:** Access Token is optional. If supplied, it is sent as `Authorization: Bearer <access-token>` without the account credential. Otherwise, use the connected OAuth credential. There is no request body or query string. The token needs `openid`; `profile` allows `name`, and `email` allows the email claims.

**Complete response shape (flat HTTP response and n8n JSON output):**
...
`sub` identifies the authorizing user. Other fields are optional/nullable according to the granted scopes. The node does not wrap these claims in `data`.

#### Get Metadata

**Endpoint:** `GET https://api.assinafy.com.br/.well-known/oauth-protected-resource`.

**Node parameters:** none. No authentication, request body, or query parameters. With an API-key credential targeting another environment, the same origin-level path is used on that environment.
...
The resource's scope list excludes `offline_access`; clients request refresh capability at the authorization server. Read the current metadata rather than assuming its scope list is fixed.

#### OAuth errors and retries

Token and revocation errors use a flat OAuth error object, surfaced as a node error:
...
| Status / error | Meaning and next step |
| --- | --- |
| `400 invalid_grant` | Expired/reused code or refresh token, invalid PKCE verifier, callback mismatch, or revoked authorization. Reconnect when necessary. |
| `400 invalid_target` | Resource indicator is unsupported or differs from the authorized value. |
